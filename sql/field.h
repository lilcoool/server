...
public:
  ...
  */
  ...
  /* store value for the duration of the current record */
  inline void store_read_value()
  {
    read_value.swap(value);
  }
  inline void set_value(uchar *data)
  {
    value.set((char*) data, 1, &my_charset_bin);    /* Set value pointer. Length not important */
    value.set_alloced();
  }
  virtual uchar *pack(uchar *to, const uchar *from, uint max_length);
  virtual const uchar *unpack(uchar *to, const uchar *from,
  ...
  inline void clear_temporary()
  {
    ...
    else if (unlikely(read_value.ptr() == (char*) tmp))
